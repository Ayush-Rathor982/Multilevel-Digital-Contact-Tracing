...
#include "GenerateInfectedList.h" 
using namespace std;
...
int infection_recovery_interval = 15;      //It's a period required to recover a infected person
...
{   
    ...
    cin >> N;
    int q = 9;         // q is the average Degree, how many average contacts a person make in a day
    float f = 0.01;    // Its a factor which reduces maximum possible contacts 
    ...
    while (1)
    {   
        if(incubation_period_count <= incubation_period) {incubation_period_count++;}
        else{
        cout<<"=====================================================";
        cout << endl << "1 => Contact data generation for current day"<<endl
        ...
        {  
            ...
            generate_data(N,day,f);   // Function for generating data
        ...
        case 2: /// For giving the Infected List
        {
            //cout<<"Infected list generation starts "<<endl; 
             GenerateInfectedList(N,day-1);   // Function for generating infected person's list
